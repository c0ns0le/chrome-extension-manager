...
myApp.controller("mainController", ["$scope", "manager", function ($scope, manager) {
    ...
    $scope.selectExtension = function (id) {
        $scope.selectedExtension = $scope.extensions[id];
    }

    $scope.enableExtension = function (id) {
        var extensionId = $scope.extensions[id].id;
        chrome.management.setEnabled(extensionId, true, function () {
            $scope.$apply(function () {
                $scope.extensions[id].enabled = true;
                $scope.selectedExtension = $scope.extensions[id];
                $scope.disabledItems = $scope.extensions.filter(function (extension) {
                    return !extension.enabled;
                });
    ...
    $scope.disableExtension = function (id) {
        var extensionId = $scope.extensions[id].id;
        chrome.management.setEnabled(extensionId, false, function () {
            $scope.$apply(function () {
                $scope.extensions[id].enabled = false;
                $scope.selectedExtension = $scope.extensions[id];
                $scope.disabledItems = $scope.extensions.filter(function (extension) {
                    return !extension.enabled;
                });
